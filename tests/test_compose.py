...
def test_join_preserves_row_index(faulty_rec):
    ...
    single = [
        M.RMS().compute(_ctx(faulty_rec, grid), grid),
    ...
    np.testing.assert_allclose(joined["rms"].to_numpy(), single[0]["rms"].to_numpy())


def test_grid_shared_across_metrics(faulty_rec):
    grid = sq.IntervalGrid.fixed(faulty_rec, 5.0)
    mf = sq.compute([M.RMS(), M.FlatFraction(), M.MaxCorrelation()], faulty_rec, grid)
...
def test_long_view_round_trips(faulty_rec):
    mf = sq.compute([M.RMS(), M.LineRatio()], faulty_rec, sq.IntervalGrid.fixed(faulty_rec, 10.0))
    long = mf.long()
    assert set(long.columns) == {"channel", "interval", "metric", "value"}
    assert len(long) == len(mf.table) * len(mf.metrics)
    back = long.pivot_table(index=["channel", "interval"], columns="metric", values="value")
    np.testing.assert_allclose(back["rms"].to_numpy(), mf.table["rms"].to_numpy(), rtol=1e-12)

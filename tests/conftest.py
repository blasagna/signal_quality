...
def clean_rec():
    ...
    ds = build_dataset(X, SFREQ, names, ["eeg"] * 6, covered=np.ones(N, dtype=bool))
...
def _rec_from(X, names, counts=None, covered=None, factor_uV=None):
    ds = build_dataset(
        ...
        ["eeg"] * len(names),
        counts=counts,
        covered=(np.ones(X.shape[1], dtype=bool) if covered is None else covered),
        factor_uV=factor_uV,
    )
    return Recording(ds, None, provenance={})
...
def clipping_rec():
    ...
    return _rec_from(signal, ["C3", "RAILED", "C4"], counts=counts, factor_uV=factor_uV)
